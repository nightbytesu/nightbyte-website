      ...
      <div className={`lg:hidden fixed inset-0 z-50 transition-all duration-500 ease-out ${
        ...
        <div 
          className={`absolute inset-0 bg-black transition-all duration-500 ${
            isMobileMenuOpen ? 'bg-opacity-60 backdrop-blur-md scale-100' : 'bg-opacity-0 backdrop-blur-none scale-105'
          }`}
          onClick={toggleMobileMenu}
        />
        
        {/* Slide-out Menu */}
        <div className={`fixed top-0 right-0 h-full w-80 max-w-sm bg-slate-900 shadow-2xl border-l border-slate-700 transform transition-all duration-500 ease-out ${
          isMobileMenuOpen ? 'translate-x-0 scale-100' : 'translate-x-full scale-95'
        }`} onClick={(e) => e.stopPropagation()}>
          
          <div className="p-6">
            {/* Mobile Menu Header */}
            <div className={`flex justify-between items-center mb-8 transform transition-all duration-400 ${
              isMobileMenuOpen ? 'translate-x-0 opacity-100 translate-y-0' : 'translate-x-12 opacity-0 -translate-y-4'
            ...
            <div className="space-y-4 mb-8">
              {items.map((item, index) => (
                <button
                  ...
                  style={{
                    transitionDelay: isMobileMenuOpen ? `${(index + 3) * 120}ms` : '0ms'
                  }}
                >
                  {item.label}
                </button>
            ...
            <Button
              ...
              style={{
                background: "linear-gradient(90.99deg, #4171F9 0.48%, #264393 99.52%)",
                transitionDelay: isMobileMenuOpen ? `${(items.length + 3) * 120}ms` : '0ms'
              }}
            >
              Contactez-nous
            </Button>
