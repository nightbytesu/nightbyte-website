      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          const progress = Math.max(
            ...
            Math.min(
              1,
              (viewportHeight - top) / (viewportHeight + elementHeight)
            )
          );
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <div className="absolute left-3 md:left-1/2 top-0 h-full w-0.5 md:w-1 md:-translate-x-1/2 transform">
        ...
        <motion.div
          className="absolute rounded-full inset-x-0 top-0 bg-linear-to-b from-blue-500 via-[#4171F9] to-blue-500 shadow-blue-500/50"
          style={{
      ...
      <div className="space-y-8 md:space-y-12">
        {memoizedChildren.map((child, index) => (
          <div key={index} className="flex items-center">
            <div className="block md:hidden w-full pl-8">{child}</div>
